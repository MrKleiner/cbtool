...
def exec():
	...
	if len(sys.argv) < 3:
		...
		print('sphere_normal.smd -> sphere_normal_conv.smd')
		return


	smd_src = Path(sys.argv[1].strip('"'))
	smd_lmap = Path(sys.argv[2].strip('"'))
	...
	assert (len(tris_src.tris) == len(tris_lmap.tris)), 'Models have different triangle count. Wtf. Aborting...'
